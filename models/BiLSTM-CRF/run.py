...
def k_fold_run():
    ...
    for train_index, dev_index in kf_data:
        ...
        test_loss, f1 = run(word_train, label_train, word_dev,
                            label_dev, vocab, device, kf_index)
...
def run(word_train, label_train, word_dev, label_dev, vocab, device, kf_index=0):
    ...
    scheduler = StepLR(optimizer, step_size=config.lr_step,
                       gamma=config.lr_gamma)
    # how to initialize these parameters elegantly
    for p in model.crf.parameters():
        _ = torch.nn.init.uniform_(p, -1, 1)
    # train and test
    train(train_loader, dev_loader, vocab, model,
          optimizer, scheduler, device, kf_index)
    with torch.no_grad():
